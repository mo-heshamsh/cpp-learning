...
int main()
{
    ...
    while (inputPass != correctPass && loginAttempts > 1)
    {
        loginAttempts--;
        cout << "\nWrong Password for " << inputName << "! Try again. " << loginAttempts << " Attempts Left. To exit, type exit.";
    ...
    while (isLogged)
    {
        ...
        else if (paymentMethod == "visa" && visaCash >= total)
        {
            visaCash -= total;
            for (int i = 0; i < 5; i++)
            {
        ...
        cout << "\npurchase successful! paid " << total << " egp.\n";
        cout << "remaining Visa Cash: " << visaCash << " egp\n";
        cout << "thank you for shopping at mo's mini market!\n";
        }
        else if (paymentMethod == "vfcash" && vfCash >= total)
        {
            vfCash -= total;
            for (int i = 0; i < 5; i++)
            {
        ...
        cout << "\npurchase successful! paid " << total << " egp.\n";
        cout << "remaining VF Cash: " << vfCash << " egp\n";
        cout << "thank you for shopping at mo's mini market!\n";
        }
        else
        {
          cout << "\ninsufficient cash balance!/Invalid Payment Method!\n";    
        }    
    ...
    break;
        ...
        break;
        case 7:
        cout << "\nPick what you want to know its balance: \n- Cash \n- Visa \n- VFCash \n";
        cin >> balanceOption;
        for (int i = 0; i < balanceOption.length(); i++)
        {
            balanceOption[i] = tolower(balanceOption[i]);
        }
        if (balanceOption == "cash")
        {
            cout << "\nYour current cash balance is: " << cash << " EGP.\n";
        }
        else if (balanceOption == "visa")
        {
            cout << "\nYour current Visa balance is: " << visaCash << " EGP.\n";
        }
        else if (balanceOption == "vfcash")
        {
            cout << "\nYour current Vodafone Cash Balance is: " << vfCash << " EGP.\n";
        }
        else
        {
        ...
        default:
        cout << "\nInvalid Option! Please rechoose from 1-7";
        break;
    }
    }
    return 0;
